 ...
 */
class FoodIdentifier(context: Context) {
    ...
    init {
        ...
        val fd = context.assets.openFd("foodid_graph.tflite")
    ...
    fun foodPercentage(picture: Bitmap, resultCallback: (result: Array<Float> )-> Unit) {
        ...
        executor.submit {
           ...
           // Log.i("foodP","read in data")
            val labelProbabilities = Array(1, { FloatArray(451) })
            ...
            var m = 0f
            var a = 0
            for (j in 0..450){ //450 inclusive. in kotlin, unlike python, endpoint is included
            ...
            Log.i("foodP","done, ready to pass back pair")
            Log.i("foodP","index: $a, percentage: $m")
